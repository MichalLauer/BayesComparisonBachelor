...
která popisuje
uložené parametry. Navíc je zde uložen parametr `deviance`, který popisuje
rozdělení **??????**. Střední hodnota posteriorního rozdělení parametru `p`
...
což dále potvrzuje uspokojivou konvergenci. **Co ten konec???**

#### Vizualizace

Software BUGS dokáže výsledky automaticky vizualizovat, což však nelze 
do prostředí R jednoduše přenést a balíček R2WinBUGS nabízí pouze základní
vizualizaci pomocí funkce `plot`, které však nejsou velmi uživatelsky 
přívětivé (viz. ukázka v příloze **REF na appendix**). Pro pokročilejší 
vizuální zobrazení lze použít balíček mcmcplots
(\parencite{Curtis2018_McmcplotsCreatePlots}).

Pro zobrazení posteriorního rozdělení jednotlivých řetězců lze použít funkce
`denplot`. Obrázek @fig-mcmcplots-posterior zobrazuje posteriorní rozdělení jednotlivých řetězců
s 89% intervalem kredibility, který je vygenerován pomocí výpisku 
\ref{mcmcplots-posteriorni}.

```{r bugs-mcmcplots-posterior}
#| echo: true
#| code-caption: Kód pro generování posteriorních rozdělení pomocí balíčku mcmcplots
#| code-label: mcmcplots-posteriorni
#| fig-cap: Posteriorní rozdělení z balíčku R2WinBUGS
#| label: fig-mcmcplots-posterior
